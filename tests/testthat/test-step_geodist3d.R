...
test_that("test step_geodist3d single location", {
  ...
  expect_true("geo_dist3d" %in% names(result))
  expect_true(inherits(result$geo_dist3d, "numeric"))
  expect_equal(sum(is.na(result$geo_dist3d)), 0)
})
...
test_that("test step_geodist3d multiple locations", {
  ...
  expect_true(inherits(result$geo_dist3d_3, "numeric"))

  expect_equal(sum(is.na(result$geo_dist3d_1)), 0)
  expect_equal(sum(is.na(result$geo_dist3d_2)), 0)
  expect_equal(sum(is.na(result$geo_dist3d_2)), 0)
})
